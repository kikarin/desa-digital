...
import { Search, Navigation } from 'lucide-vue-next';
...
const setMarker = (lat: number, lng: number) => {
    ...
    const popupText = props.markerPopupText || 'Lokasi Terpilih';
    marker.bindPopup(`<b>${popupText}</b>`).openPopup();
    map.setView([lat, lng], 16);

    const latStr = lat.toString();
    const lngStr = lng.toString();
    emit('update:latitude', latStr);
    emit('update:longitude', lngStr);
};
...
onMounted(() => {
    ...
    // Set view berdasarkan props atau default
...
watch(
    ...
    ([newLat, newLng]) => {
        if (map && newLat && newLng) {
            ...
            if (!isNaN(lat) && !isNaN(lng)) {
                if (!marker || marker.getLatLng().lat !== lat || marker.getLatLng().lng !== lng) {
                    setMarker(lat, lng);
                }
            }
        }
...
<template>
    <Card>
        <CardHeader>
            <CardTitle class="text-lg">Titik Lokasi</CardTitle>
        </CardHeader>
        <CardContent class="space-y-4">
            ...
            <!-- Koordinat Display -->
            <p v-if="latitude && longitude" class="text-xs text-muted-foreground">
