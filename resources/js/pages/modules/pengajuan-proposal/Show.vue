...
const breadcrumbs = [
    ...
    { title: 'Detail Pengajuan Proposal', href: `/pengajuan-proposal/${props.item.id}` },
];


const fields = [
    { label: 'Kategori Proposal', value: props.item.kategori_proposal_nama || '-' },
    { label: 'Pemohon', value: props.item.resident_nama && props.item.resident_nik ? `${props.item.resident_nama} (${props.item.resident_nik})` : '-' },
    { label: 'Nomor Telepon Pengaju', value: props.item.nomor_telepon_pengaju || '-' },
...
const handleVerifikasi = () => {
    router.visit(`/pengajuan-proposal/${props.item.id}/verifikasi`);
};

const handleExportPdf = () => {
    if (props.item.status === 'disetujui') {
        window.location.href = `/pengajuan-proposal/${props.item.id}/export-pdf`;
    }
};

const handlePreviewPdf = () => {
    if (props.item.status === 'disetujui') {
        window.open(`/pengajuan-proposal/${props.item.id}/preview-pdf`, '_blank');
    }
};

const isDisetujui = computed(() => {
    return props.item.status === 'disetujui';
});
...
<template>
    <PageShow
        ...
        :back-url="'/pengajuan-proposal'"
    >
        <template #custom-action>
            <Button
                v-if="isDisetujui && can?.['Preview PDF']"
            ...
            <Button
                v-if="isDisetujui && can?.['Export PDF']"
