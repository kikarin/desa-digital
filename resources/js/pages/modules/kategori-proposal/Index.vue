...
const actions = (row: any) => [
    {
        label: 'Detail',
        onClick: () => router.visit(`/data-master/kategori-proposal/${row.id}`),
        permission: props.can?.Detail,
    },
    {
        label: 'Edit',
        onClick: () => router.visit(`/data-master/kategori-proposal/${row.id}/edit`),
...
const deleteSelected = async () => {
    ...
    try {
        const response = await axios.post('/data-master/kategori-proposal/destroy-selected', {
            ids: selected.value,
        });
...
const deleteRow = async (row: any) => {
    await router.delete(`/data-master/kategori-proposal/${row.id}`, {
        onSuccess: () => {
...
<template>
    <PageIndex
        ...
        :columns="columns"
        :create-url="'/data-master/kategori-proposal/create'"
        :actions="actions"
        :selected="selected"
        @update:selected="(val: number[]) => (selected = val)"
        :on-delete-selected="deleteSelected"
        api-endpoint="/api/kategori-proposal"
        ref="pageIndex"
